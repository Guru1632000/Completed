...
import { renderQuestionText, renderTextWithMarkdown } from './utils';
...
const TestView: React.FC<TestViewProps> = ({ questions, topic, onFinishTest, activeExamType, mode, durationInMinutes, currentTestTopicId }) => {
  ...
  const isBankPrelimsMock = useMemo(() => {
    return topic.name.includes('Prelims Full Mock Test') && !topic.name.includes('RRB');
  }, [topic.name]);
  ...
  useEffect(() => {
    try {
      if (mode === 'test') {
        const savedProgressRaw = localStorage.getItem('aiexam-test-progress');
        const savedSessionRaw = localStorage.getItem('aiexam-test-session');
    ...
    } catch (e) { console.error("Failed to load test progress", e); localStorage.removeItem('aiexam-test-progress'); }
  ...
  useEffect(() => {
    if (mode === 'test') {
        ...
        localStorage.setItem('aiexam-test-session', JSON.stringify(sessionData));
    }
  ...
  useEffect(() => {
    if (mode === 'test') {
      ...
      localStorage.setItem('aiexam-test-progress', JSON.stringify(progress));
    }
              ...
              <div className={`bg-black/20 rounded-2xl shadow-xl border ${theme.border} overflow-hidden`}>
                  <div className="p-8 question-container">
                      <div key={currentQuestionIndex} className={direction === 'next' ? 'question-enter-next' : 'question-enter-prev'}>
                          ...
                          <div className="mb-8">{renderQuestionText(currentQuestion, theme)}{currentQuestion.isPYQ && <span className="text-xs font-bold bg-amber-400/20 text-amber-300 px-2 py-0.5 rounded-full ml-2">PYQ</span>}</div>
