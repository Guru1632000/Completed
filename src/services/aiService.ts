    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
            ...
            if (error instanceof Error && (error.message.includes("[400]") || error.message.includes("SAFETY"))) {
                 console.error("Received a non-retriable client-side error. Halting retries.", error);
                 throw new Error("The AI model rejected the request due to a client-side issue (e.g., prompt safety or invalid arguments). Please check the console for details.");
            }

            if (attempt === 3) {
                 console.error("All 3 attempts to call the AI model failed. Giving up.", error);
                 throw error;
            }
...
const callGeminiForText = async (
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
            ...
            console.warn(`Text generation attempt ${attempt} failed:`, (error as Error).message);
            if (attempt === 3) {
                console.error("All 3 attempts to get text from AI failed.", error);
                throw error;
            }
...
export const generateSSCMockTestQuestions = async (exam: SSCExamFilterType, tier: 'Tier-1' | 'Tier-2' | 'Paper-I', difficulty: Difficulty, updateProgress: (progress: number, text: string, currentStepIndex?: number) => void, signal?: AbortSignal, customConfig?: ExamPatternConfig): Promise<Question[]> => {
    ...
    for (const [index, [sectionName, subTopics]] of sections.entries()) {
        ...
        for (const job of jobs.filter(j => j.count > 0)) {
            const relevantSyllabusUnit = fullSyllabus.find(u => u.title.includes(sectionName));
            if (!relevantSyllabusUnit) continue;
            
            const topicsForGeneration = relevantSyllabusUnit.topics.filter(t => job.topicKeywords.some(kw => t.name.toLowerCase().includes(kw.toLowerCase())));
            if (topicsForGeneration.length === 0) continue;
...
// FIX: Corrected property names in mock study plan to match the StudyPlanItem interface. 'start' is now 'startDay' and 'end' is now 'endDay'.
        return [
