...
const App: React.FC = () => {
  ...
  const handleDownloadPdf = useCallback(async (result: TestResult, options?: { showScore?: boolean }) => {
    ...
    const A3_PAGE_WIDTH_PX = pdf.internal.pageSize.getWidth();
    const A3_PAGE_HEIGHT_PX = pdf.internal.pageSize.getHeight();
    const PADDING_TOP_BOTTOM = 60;
    const CONTENT_MAX_HEIGHT = A3_PAGE_HEIGHT_PX - (PADDING_TOP_BOTTOM * 2);
    const PADDING_BETWEEN_QUESTIONS_PX = 40;
    ...
    const css = `
        ...
        .page { width: ${A3_PAGE_WIDTH_PX}px; padding: ${PADDING_TOP_BOTTOM}px; margin: 0 auto; display: flex; flex-direction: column; justify-content: flex-start; }
        .title-page { display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; height: ${A3_PAGE_HEIGHT_PX}px; box-sizing: border-box; }
    ...
    const createHtmlForQuestionCard = (q: Question, index: number): string => {
        if (!q) return '';
    ...
    try {
        ...
        tempContainer.style.width = `${A3_PAGE_WIDTH_PX}px`;
        tempContainer.innerHTML = `<style>${css}</style>${titlePageHtml}`;
        document.body.appendChild(tempContainer);
        
        const titleCanvas = await window.html2canvas(tempContainer.querySelector('.title-page') as HTMLElement, { scale: 2, useCORS: true });
        
        const pdfWidth = pdf.internal.pageSize.getWidth();
        const projectedImgHeight = titleCanvas.height * pdfWidth / titleCanvas.width;

        pdf.addImage(titleCanvas.toDataURL('image/png'), 'PNG', 0, 0, pdfWidth, projectedImgHeight);
        document.body.removeChild(tempContainer);

        // --- Question Pages (Auto Layout - Optimized) ---
        let questionIndex = 0;
        const totalQuestions = result.questions.length;

        while (questionIndex < totalQuestions) {
            const questionsForPage: Question[] = [];
            const questionIndicesForPage: number[] = [];
            let currentHeightOnPage = 0;

            const measureContainer = document.createElement('div');
            measureContainer.style.position = 'absolute';
            measureContainer.style.left = '-9999px';
            measureContainer.style.width = `${A3_PAGE_WIDTH_PX - (PADDING_TOP_BOTTOM * 2)}px`;
            document.body.appendChild(measureContainer);

            for (let i = questionIndex; i < totalQuestions; i++) {
                if (!result.questions[i]) continue;
                const questionHtml = createHtmlForQuestionCard(result.questions[i], i);
                
                measureContainer.innerHTML = `<style>${css}</style>${questionHtml}`;
                const cardElement = measureContainer.querySelector('.question-card') as HTMLElement;
                if (!cardElement) continue;
                
                const cardHeight = cardElement.scrollHeight;
                
                const isFirstOnPage = questionsForPage.length === 0;
                const heightWithNewCard = currentHeightOnPage + cardHeight + (isFirstOnPage ? 0 : PADDING_BETWEEN_QUESTIONS_PX);

                if (!isFirstOnPage && heightWithNewCard > CONTENT_MAX_HEIGHT) {
                    break;
                }

                questionsForPage.push(result.questions[i]);
                questionIndicesForPage.push(i);
                currentHeightOnPage = heightWithNewCard;
            }
            ...
            if (questionsForPage.length > 0) {
                ...
                pageContainer.style.width = `${A3_PAGE_WIDTH_PX}px`;
                document.body.appendChild(pageContainer);

                const pageContentHtml = questionsForPage.map((q, idx) => 
                    createHtmlForQuestionCard(q, questionIndicesForPage[idx])
                ).join(`<div style="height: ${PADDING_BETWEEN_QUESTIONS_PX}px;"></div>`);
                
                pageContainer.innerHTML = `<style>${css}</style><div class="page">${pageContentHtml}</div>`;
                const pageElementToRender = pageContainer.querySelector('.page') as HTMLElement;
                
                const images = Array.from(pageElementToRender.querySelectorAll('img, image'));
                ...
                const pageCanvas = await window.html2canvas(pageElementToRender, { scale: 2, useCORS: true, windowHeight: pageElementToRender.scrollHeight });
                
                pdf.addPage();
                const canvasHeight = pageCanvas.height;
                const canvasWidth = pageCanvas.width;
                const imgHeight = canvasHeight * pdfWidth / canvasWidth;
                const yPosition = (A3_PAGE_HEIGHT_PX > imgHeight) ? (A3_PAGE_HEIGHT_PX - imgHeight) / 2 : 0;
                
                pdf.addImage(pageCanvas.toDataURL('image/png'), 'PNG', 0, yPosition, pdfWidth, imgHeight, undefined, 'FAST');
                
                document.body.removeChild(pageContainer);
            }
            
            questionIndex += questionsForPage.length;

            if (questionsForPage.length === 0 && questionIndex < totalQuestions) {
                console.warn(`A single question (${questionIndex}) is too large to fit on one page. It will be rendered on its own page, potentially overflowing.`);
                questionIndex++;
            }
        }
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'dashboard':
          return <DashboardView history={testHistory} testTopics={testTopics} activeExamType={activeExamType} />;
      case 'history':
        return <HistoryView 
                  history={testHistory} 
