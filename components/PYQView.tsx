...
// FIX: Imported the newly exported PYQ_EXAM_GROUPS constant.
...
const PYQView: React.FC<PYQViewProps> = ({ onGenerateTest, activeExamType }) => {
    // FIX: Dynamically get groups based on the active exam type.
    const groups = PYQ_EXAM_GROUPS[activeExamType as keyof typeof PYQ_EXAM_GROUPS] || [];
    const [selectedYear, setSelectedYear] = useState<number>(currentYear);
    // FIX: Initialize with the first group from the dynamic list.
    ...
    // FIX: Add effect to reset selected group when exam type or available groups change.
    useEffect(() => {
        setSelectedGroup(groups[0] || '');
    }, [activeExamType, groups]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setError('');
        // FIX: Pass activeExamType to the generation handler.
