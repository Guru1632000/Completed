...
const parseTopicsWithDash = (text: string): string[] => {
    ...
    // Find the start of the content. It's the first line that doesn't look like a title.
    ...
    if (contentStartIndex === -1) {
        // Fallback for simple cases if findIndex fails
        contentStartIndex = lines.length > 1 ? 1 : 0;
    }

    let content = lines.slice(contentStartIndex).join(' '); 

    // Special handling for the list of leaders
    ...
    return content
        ...
        .filter(line => 
            line.length > 1 && // Filter out single characters or empty strings
            !line.toLowerCase().includes('a.d.') && // Filter out "A.D." artifacts
            line.toLowerCase() !== 'others' // Filter out "others"
        );
};


const parseLiteraryTopics = (text: string): string[] => {
    // Skip title, join everything else into a single line for easier processing
    const content = text.trim().split('\n').slice(1).join(' '); 
    
    // Remove the 'Poems:' and 'Prose:' labels
    const allTopicsText = content
        .replace(/Poems:/g, '')
        .replace(/Prose:/g, '');
        
    // Split by semicolon, clean up, and filter out empty items
    return allTopicsText
        .split(';')
        .map(topic => topic.replace(/–/g, '-').trim()) // Normalize dash and trim whitespace
        .filter(Boolean);
};
...
// Specific Syllabus combinations for different groups/stages
export const TNPSC_GROUP1_PRELIMS_SYLLABUS: SyllabusUnit[] = [
    gsUnit1, gsUnit2, gsUnit3, gsUnit4, gsUnit5, gsUnit6,
    aptitudeUnit, reasoningUnit,
];

export const TNPSC_GROUP2_4_PRELIMS_SYLLABUS: SyllabusUnit[] = [
    gsUnit1, gsUnit2, gsUnit3, gsUnit4, gsUnit5, gsUnit6,
    aptitudeUnit, reasoningUnit,
...
export const TNPSC_MAINS_SYLLABUS: SyllabusUnit[] = [
    mainsPaper2Unit1, mainsPaper2Unit2, mainsPaper2Unit3,
    mainsPaper3Unit1, mainsPaper3Unit2, mainsPaper3Unit3,
    mainsPaper4Unit1, mainsPaper4Unit2, mainsPaper4Unit3,
];
